...
function LocustSwarmPhysics( event )
    ...
    unit:OnPhysicsFrame(function(unit)
        ...
        if unit.state == "acquiring" then
            ...
            enemies = FindUnitsInRadius(caster:GetTeamNumber(), source, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, abilityTargetType,
                                        DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_ANY_ORDER, false)
...
function LocustDebugPrint(...)
    if Debug then print('[LocustSwarm] ' .. ...) end
end

function LocustDebugDraw(center,vRgb,a,rad,ztest,duration)
    if Debug then DebugDrawCircle(center,vRgb,a,rad,ztest,duration) end
end